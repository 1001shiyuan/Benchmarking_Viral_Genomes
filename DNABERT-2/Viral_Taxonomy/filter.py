...
def process_full_sequences(full_seq_dir, thresholds):
    ...
    for label_type in ['order', 'family', 'genus']:
        ...
        # Create output directory
        output_dir = f'{label_type}_full_embed'
...
def process_contig_data(valid_labels, contig_dir, contig_lengths):
    ...
    for label_type in ['order', 'family', 'genus']:
        ...
        for split in ['train', 'val']:
            ...
            output_dir = f'{label_type}_contig_embed'
        ...
        for length in contig_lengths:
            ...
            length_output_dir = os.path.join(f'{label_type}_contig_embed', f'length_{length}')
...
def main():
    # Define paths
    FULL_SEQ_DIR = 'full_embed'
    CONTIG_DIR = 'contig_embed'
    ...
    for label_type in ['order', 'family', 'genus']:
        os.makedirs(f'{label_type}_contig_embed', exist_ok=True)
        source_encoder = os.path.join(CONTIG_DIR, 'label_encoders.npy')
        if os.path.exists(source_encoder):
            shutil.copy2(
                source_encoder,
                os.path.join(f'{label_type}_contig_embed', 'label_encoders.npy')
            )
    
    # Process full sequences to determine valid labels
    valid_labels = process_full_sequences(FULL_SEQ_DIR, THRESHOLDS)
    
    # Process contig data using the valid labels
    process_contig_data(valid_labels, CONTIG_DIR, CONTIG_LENGTHS)
