...
# Set environment variable to enable tokenizers parallelism
os.environ["TOKENIZERS_PARALLELISM"] = "true"

# Use spawn instead of fork to avoid deadlocks with tokenizer parallelism
# This must be done before creating any processes/threads
if __name__ == '__main__':
    # Only set start method if it hasn't been set already (to avoid errors)
    try:
...
class PrecomputeArguments:
    ...
    num_workers: int = field(default=4)  # Number of DataLoader workers
    output_dir: str = field(default="/work/sgk270/dnabert2_task2_new/contig_embed")
    full_output_dir: str = field(default="/work/sgk270/dnabert2_task2_new/full_embed")
...
def extract_embeddings(base_model, dataset, tokenizer, output_dir, split_name, batch_size, num_workers, device, stats):
    """Extract CLS embeddings and save with sequence IDs."""
    log_with_time(f"Starting embedding extraction for {split_name} split with {len(dataset)} sequences...")
    base_model.eval()
    all_embeddings = []
    all_sequence_ids = []

    # Use multiple workers with spawn method to enable both forms of parallelism
    dataloader = DataLoader(
        dataset, 
        batch_size=batch_size, 
        shuffle=False,
        num_workers=num_workers,
        pin_memory=True,  # This speeds up host-to-device transfers
        persistent_workers=True if num_workers > 0 else False  # Keep workers alive between batches
    )
    
    total_batches = len(dataloader)
    log_with_time(f"Created dataloader with {total_batches} batches (batch size: {batch_size}, workers: {num_workers})")
...
def process_train_val_contigs(base_model, tokenizer, data_args, output_dir, precompute_args, device, stats):
    ...
    for split in ['train', 'val']:
        ...
        try:
            ...
            log_with_time(f"Processing {len(sequences)} sequences for {split} split")
            ...
            extract_embeddings(
                ...
                precompute_args.num_workers,
                device,
                stats
            )
        except FileNotFoundError as e:
            log_with_time(f"Warning: {e}. Skipping {split} split.")
...
def process_test_contigs(base_model, tokenizer, data_args, output_dir, precompute_args, device, stats):
    ...
    for contig_length in data_args.contig_lengths:
        ...
        try:
            ...
            log_with_time(f"Processing {len(sequences)} sequences for test length {contig_length}")
            ...
            log_with_time(f"Warning: {e}. Skipping test for length {contig_length}.")
        except Exception as e:
...
def process_full_sequences(base_model, tokenizer, data_path, output_dir, precompute_args, device, stats):
    ...
    for split in ['train', 'val', 'test']:
        ...
        try:
            ...
            log_with_time(f"Processing {len(sequences)} sequences for full sequence {split} split")
            ...
            log_with_time(f"Warning: {e}. Skipping full sequence {split} split.")
        except Exception as e:
...
def main():
    ...
    log_with_time("Creating output directories...")
    ...
    log_with_time("Loading tokenizer and model...")
    log_with_time(f"Tokenizer parallelism is enabled: {os.environ.get('TOKENIZERS_PARALLELISM', 'Not set')}")
    ...
    log_with_time("Model loaded and moved to device")
    ...
    if contig_stats['total'] > 0:
        log_with_time(f"Contigs: {contig_stats['truncated']} out of {contig_stats['total']} sequences truncated ({contig_stats['truncated']/contig_stats['total']*100:.2f}%)")
    if full_seq_stats['total'] > 0:
        log_with_time(f"Full Sequences: {full_seq_stats['truncated']} out of {full_seq_stats['total']} sequences truncated ({full_seq_stats['truncated']/full_seq_stats['total']*100:.2f}%)")
