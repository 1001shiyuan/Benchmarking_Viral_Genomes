...
#####################################################################################################
    model_name_or_path: Optional[str] = field(default="AIRI-Institute/gena-lm-bigbird-base-sparse-t2t")
#####################################################################################################
...
class PrecomputeArguments:
    ...
    output_dir: str = field(default="/work/sgk270/genalm_task3_new/contig_embed")
    full_output_dir: str = field(default="/work/sgk270/genalm_task3_new/full_embed")
...
##################################################################################################################
                if len(tokens) > 4096:
                    stats['truncated'] += 1
            inputs = tokenizer(sequences, return_tensors='pt', padding=True, truncation=True, max_length=4096)
##################################################################################################################
...
def main():
    log_with_time("Starting GenaLM embedding extraction for lifestyle classification (Task 3)")
