...
class PrecomputeArguments:
    ...
    output_dir: str = field(default="/work/sgk270/genalm_task2/contig_embed")
    full_output_dir: str = field(default="/work/sgk270/genalm_task2/full_embed")
...
def extract_embeddings(model, dataset, tokenizer, output_dir, split_name, batch_size, device, stats, label_encoders):
    """Extract embeddings and save with multiple labels."""
    model.eval()
    all_embeddings = []
    all_labels = {
        'Order': [],
    ...
    with torch.no_grad():
        for sequences, sequence_ids, labels_dict, _ in dataloader:
            ...
            # Adapt to handle different output structures
            if hasattr(outputs, 'last_hidden_state'):
                embeddings = outputs.last_hidden_state[:, 0, :]
            elif hasattr(outputs, 'hidden_states'):
                embeddings = outputs.hidden_states[-1][:, 0, :]
            else:
                raise AttributeError("Cannot find either last_hidden_state or hidden_states in model outputs")
...
def main():
    ...
    # Load model according to Hugging Face documentation
    model = AutoModel.from_pretrained(
        model_args.model_name_or_path,
        trust_remote_code=True,
        output_hidden_states=True  # Request hidden states to ensure they're available
    ).to(device)
