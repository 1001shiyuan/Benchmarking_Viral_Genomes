...
def plot_combined_training_history(output_dir, label_type="order"):
    ...
    # Add a global title
    fig.suptitle(f'GENA-LM Order Classification Training Metrics', fontsize=16)
    
    # Save the combined figure
    plt.tight_layout(rect=[0, 0, 1, 0.96])  # Adjust for the suptitle
    plt.savefig(os.path.join(output_dir, f'genalm_{label_type}_training_curves.png'), dpi=300, bbox_inches='tight')
    plt.close()
...
def plot_metrics_by_length(results_df, output_dir, label_type="order"):
    ...
    plt.tight_layout()
    fig.suptitle(f'GENA-LM Order Classification Performance by Sequence Length (Test Set)', y=1.02, fontsize=16)
    plt.savefig(os.path.join(output_dir, f'genalm_{label_type}_metrics_by_length.png'), dpi=300, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    print(f"Analyzing GENA-LM {label_type.capitalize()} Classification Results")
    ...
    if results:
        ...
        # Save results to CSV
        results_df.to_csv(os.path.join(output_dir, f'genalm_{label_type}_results.csv'), index=False)
