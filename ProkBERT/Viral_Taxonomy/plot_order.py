...
def calculate_metrics(predictions, true_labels, probabilities):
    ...
    return {
        ...
        'valid_orders': class_count
    }

def plot_combined_training_history(output_dir, label_type="order"):
    ...
    # Add a global title
    fig.suptitle(f'ProkBERT Order Classification Training Metrics', fontsize=16)
    
    # Save the combined figure
    plt.tight_layout(rect=[0, 0, 1, 0.96])  # Adjust for the suptitle
    plt.savefig(os.path.join(output_dir, f'prokbert_{label_type}_training_curves.png'), dpi=300, bbox_inches='tight')
    plt.close()

def plot_metrics_by_length(results_df, output_dir, label_type="order"):
    ...
    plt.tight_layout()
    fig.suptitle(f'ProkBERT Order Classification Performance by Sequence Length (Test Set)', y=1.02, fontsize=16)
    plt.savefig(os.path.join(output_dir, f'prokbert_{label_type}_metrics_by_length.png'), dpi=300, bbox_inches='tight')
    plt.close()

def main():
    # Configure parameters
    label_type = 'order'
    output_dir = 'order_output'  # Directory containing results and where plots will be saved
    contig_lengths = [500, 1000, 3000, 5000, 10000]
    
    print(f"Analyzing ProkBERT {label_type.capitalize()} Classification Results")
    ...
    if results:
        ...
        # Save results to CSV
        results_df.to_csv(os.path.join(output_dir, f'prokbert_{label_type}_results.csv'), index=False)
        ...
        # Print number of orders for each length group
        print("\nNumber of orders in each length group:")
        print(results_df[['dataset', 'length_group', 'valid_orders']].to_string(index=False))
    else:
