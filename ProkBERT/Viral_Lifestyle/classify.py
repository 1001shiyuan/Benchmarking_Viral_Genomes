...
def load_embeddings(embeddings_dir, filename):
    ...
    # Use lifestyle labels for classification
...
def add_test_labels_to_map(length_test_dirs, label_map, unique_labels):
    ...
    for test_dir in length_test_dirs:
        ...
        if os.path.exists(test_path):
            ...
            test_labels = data['lifestyle_labels']
...
def train_and_evaluate(embeddings_dir, output_dir, model_prefix, batch_size, learning_rate, num_epochs, early_stop_patience, length_test_dirs=None):
    """Train and evaluate a model for lifestyle data."""
    ...
    if length_test_dirs is not None:
        unique_labels, label_map = add_test_labels_to_map(length_test_dirs, label_map, unique_labels)
    
    valid_classes = label_encoders['lifestyle']['classes'][unique_labels]
    num_classes = len(unique_labels)
    print(f"Number of unique lifestyle classes for {model_prefix}: {num_classes}")
    print(f"Lifestyle classes: {valid_classes}")
    ...
    }).to_csv(os.path.join(output_dir, f'lifestyle_{model_prefix}_training_history.csv'), index=False)
    ...
    if length_test_dirs:
        for test_dir in length_test_dirs:
            ...
            if os.path.exists(test_file):
                ...
                np.save(os.path.join(output_dir, f'lifestyle_contigs_{length_id}_eval_results.npy'), simplified_results)
    else:
        ...
        if os.path.exists(test_file):
            ...
            np.save(os.path.join(output_dir, f'lifestyle_full_eval_results.npy'), simplified_results)
...
def process_contig_lengths(args, early_stop_patience, batch_size, learning_rate):
    """Process contigs with combined training and length-specific testing."""
    contig_lengths = [500, 1000, 3000, 5000, 10000]
    # Updated to use prokbert task3 paths
    base_dir = "/work/sgk270/prokbert_task3"
    combined_dir = os.path.join(base_dir, "lifestyle_contig_embed")
    ...
    if not length_test_dirs:
        print(f"WARNING: No length-specific test directories found for lifestyle data")
    
    # Train on combined data and evaluate on each length-specific test set
    print(f"\n{'='*50}")
    print(f"Training on combined contigs for lifestyle classification")
    print(f"{'='*50}")
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="/work/sgk270/prokbert_task3/lifestyle_output")
    ...
    print("\nRunning classification for ProkBERT lifestyle embeddings")
    os.makedirs(args.output_dir, exist_ok=True)
    
    # Process contigs
    process_contig_lengths(args, args.early_stop_patience, batch_size, learning_rate)
    
    # Process full sequences
    # Updated to use prokbert task3 paths
    base_dir = "/work/sgk270/prokbert_task3"
    full_embeddings_dir = os.path.join(base_dir, "lifestyle_full_embed")
