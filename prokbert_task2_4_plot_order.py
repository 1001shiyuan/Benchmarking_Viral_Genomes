...
def plot_combined_training_history(output_dir, label_type="order"):
    ...
    fig.suptitle("Order Classification Training Curves", fontsize=16)
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    plt.savefig(os.path.join(output_dir, "order_training_curves_prokbert.png"), dpi=300)
    plt.close()
...
def plot_metrics_by_length(results_df, output_dir, label_type="order"):
    ...
    fig.suptitle("Order Classification Performance by Sequence Length", fontsize=16, y=1.02)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "order_metrics_by_length_prokbert.png"), dpi=300)
    plt.close()

def main():
    label_type = 'order'
    output_dir = 'order_output'  # <-- LOCAL folder
