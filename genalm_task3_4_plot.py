...
def calculate_metrics(predictions, true_labels, probabilities):
    ...
    return {
        ...
        'valid_lifestyles': len(unique_classes)
    }

def plot_combined_training_history(output_dir, label_type="lifestyle"):
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(18, 6))
    colors = ['#1f77b4', '#ff7f0e']  # Blue for contigs, orange for full

    contigs_file = os.path.join(output_dir, f"{label_type}_contigs_training_history.csv")
    ...
    fig.suptitle(f"{label_type.capitalize()} Classification Training Curves", fontsize=16)
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    plt.savefig(os.path.join(output_dir, f"{label_type}_training_curves_genalm.png"), dpi=300)
    plt.close()

def plot_metrics_by_length(results_df, output_dir, label_type="lifestyle"):
    ...
    fig.suptitle(f"{label_type.capitalize()} Classification Performance by Sequence Length", fontsize=16, y=1.02)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"{label_type}_metrics_by_length_genalm.png"), dpi=300)
    plt.close()

def main():
    label_type = 'lifestyle'
    output_dir = 'lifestyle_output'  # <- Local folder with ProkBERT results
    ...
    full_file = os.path.join(output_dir, f"lifestyle_full_eval_results.npy")
    if os.path.exists(full_file):
    ...
    if results:
        ...
        print(results_df[['dataset', 'length_group', 'valid_lifestyles']].to_string(index=False))
    else:
